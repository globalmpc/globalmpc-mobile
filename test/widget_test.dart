...
void main() {
  ...
  test('MPC facts follow DEC-01 resolution (2026-07-20: BSC)', () {
    expect(MpcFacts.totalSupply, 10000000000);
    // DEC-01 was resolved on 2026-07-20: BSC for the app.
    // Changing chain again requires an explicit new decision, not a drive-by
    ...
    // The legacy Polygon contract (where all pre-migration holders are) stays
    // pinned for the migration-state UI.
    expect(
      MpcFacts.legacyPolygonContract.toLowerCase(),
  ...
  test('mock repository returns grounded projects and wallet', () async {
    ...
    // Competent Person signed report exists. Guards against re-introducing the
    // overstatement the public site's wording invites.
  ...
  test('Earn is honest — planned capabilities, no invented rate', () {
    // Grounded in the product direction: staking + farming, both gated on KYC + listing.
    expect(
