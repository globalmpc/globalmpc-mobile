...
///
/// These run against the in-memory secure-storage test platform, which is the
/// only tier the store has on any platform.
void main() {
  ...
  Future<String?> storedPin() async =>
      const FlutterSecureStorage().read(key: 'wallet_device_pin_v1');
  ...
  group('wallet state', () {
    ...
    test('keeps the phrase and address in secure storage only', () async {
      await store.saveWalletSecrets(
        mnemonic: 'abandon abandon abandon',
        address: '0x9858EfFD232B4033E47d90003D41EC34EcaEda94',
      );

      expect(await store.readMnemonic(), 'abandon abandon abandon');
      expect(
        await store.walletAddress(),
        '0x9858EfFD232B4033E47d90003D41EC34EcaEda94',
      );

      // The phrase may live under exactly one key, so no incidental copy is
      // left behind for a storage dump to pick up.
      final all = await const FlutterSecureStorage().readAll();
      final carriers = all.entries
          .where((e) => e.value.contains('abandon'))
          .map((e) => e.key)
          .toList();
      expect(carriers, ['wallet_mnemonic_v1']);
    });

    test('clear() removes the phrase', () async {
      await store.saveWalletSecrets(
        mnemonic: 'abandon abandon abandon',
        address: '0x9858EfFD232B4033E47d90003D41EC34EcaEda94',
      );
      ...
      expect(await store.walletAddress(), isNull);
    });
  });
}
