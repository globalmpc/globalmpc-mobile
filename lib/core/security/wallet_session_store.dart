...
///
/// Storage tiers (spec S4/S5):
/// - iOS/Android: everything in [FlutterSecureStorage], hardened so entries
///   are device-bound (`first_unlock_this_device`) and never restored to a
///   different device by OS/cloud backup.
/// - macOS (dev/test shell, not a wallet target): PIN flags and the public
///   address fall back to [SharedPreferences]; the mnemonic is NEVER written
///   there, so desktop builds are watch-only after a restart.
class WalletSessionStore {
  ...
  /// enters platform secure storage; on macOS only the address is kept.
  ...
  /// (spec S1/T16). Null on macOS and when no wallet is configured.
  ...
  Future<void> clear() async {
    if (Platform.isMacOS) {
      ...
      await prefs.remove(_pinFailuresKey);
