...
///
/// Chain and contract per DEC-01 resolution (2026-07-20): BSC for the app,
/// not Polygon. The app describes the BSC token, which the official site
/// also shows.
///
/// Context that still matters: every existing holder's MPC (89 wallets, the
/// full wind-down payout history) sits on the legacy Polygon contract
/// 0x2d854416d2749b1f0eb8a4b2ab9027989f2ba262. Until a migration/swap is
/// defined, a holder's BSC balance is zero, so the balance UI must explain
/// the migration state rather than show a silent zero. Migration plan:
/// requested, pending.
class MpcFacts {
  const MpcFacts._();
  ...
  /// ERC-3643 is the whitepaper's PLANNED issuance standard (DEC-02), never a
  /// claim about the live token. Render it only via 'facts.plannedStandard'.
  static const String tokenStandard = 'ERC-3643';
  static const String explorerBase = 'https://bscscan.com';

  /// Legacy Polygon MPC contract, where all pre-migration holders are.
  /// Kept for the migration-state UI and reconciliation, never as "the" token.
  static const String legacyPolygonContract =
      '0x2d854416d2749b1f0eb8a4b2ab9027989f2ba262';
